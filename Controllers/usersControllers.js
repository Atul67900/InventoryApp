...
exports.userpost = async(req,res)=>{
    ...
    const {pname, pcode, pquantity, pbin, status} = req.body;

    if(!pname || !pcode || !pquantity || !pbin || !status || !file){
    ...
    try {
        ...
        if(preuser){
            ...
            const productData = new products({
                pname, pcode, pquantity, pbin, status, profile:file, datecreated
            });
...
exports.productedit = async(req,res)=>{
    const {id} = req.params;
    const {pname, pcode, pquantity, pbin, status, user_profile} = req.body;
    ...
    try {
        const updateProduct = await products.findByIdAndUpdate({_id:id},{
            pname, pcode, pquantity, pbin, status, profile:file, dateUpdated
        },{
            new:true
        });
...
exports.productquantity = async(req,res)=>{
    ...
    try {
        const productquantityUpdate = await products.findByIdAndUpdate({_id:id},{pquantity:data},{new:true});
        res.status(200).json(productquantityUpdate)
    } catch (error) {
        res.status(401).json(error)
    }
}
        ...
        if(productsData.length > 0){
            productsData.map((product)=>{
                csvStream.write({
                    ...
                    ProductBin:product.pbin ? product.pbin : "-",
